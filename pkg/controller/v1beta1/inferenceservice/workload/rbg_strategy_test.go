...
func TestRBGStrategy_ReconcileWorkload_CreatesRBGAndHPAandRBAC(t *testing.T) {
	...
	// RBAC was reconciled for engine (ServiceAccount only) and router (SA + Role + RoleBinding).
	var engineSA corev1.ServiceAccount
	require.NoError(t, c.Get(context.Background(), types.NamespacedName{Name: "my-isvc-engine", Namespace: "default"}, &engineSA))
	...
	// HPA was created for each RawDeployment role and targets the role's deployment by name.
	...
	assert.Equal(t, "Deployment", engineHPA.Spec.ScaleTargetRef.Kind)
...
func TestRBGStrategy_ReconcileWorkload_PDDisaggregated(t *testing.T) {
	...
	// Every component gets its own ServiceAccount.
	for _, comp := range []string{"engine", "decoder", "router"} {
		var sa corev1.ServiceAccount
		require.NoError(t, c.Get(context.Background(), types.NamespacedName{Name: "pd-isvc-" + comp, Namespace: "default"}, &sa),
			"expected ServiceAccount for %s", comp)
	}
	// Router additionally gets Role + RoleBinding.
	var routerRole rbacv1.Role
	...
	// HPAs are created for RawDeployment roles only. Decoder is MultiNode
	// and is therefore scaled by the RBG ScalingAdapter, not an HPA.
	var engineHPA autoscalingv2.HorizontalPodAutoscaler
